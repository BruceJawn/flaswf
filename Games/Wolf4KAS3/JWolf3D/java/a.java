...
public class a extends Applet implements Runnable {
  ...
  public void run() {
    ...
    // sprite colors
    final int[] PALETTE = {
      0xFCC4A4, 0xD08050, 0x0000AD, 0xFF4F4F, 0xA8A8A8, 0x2C2C2C, 0xFCF400,
      0xFFFFFF, 0xFF0000, 0x4F4F00, 0xE4DA00
    };    
      ...
      for(int offset = 0; offset < width; offset += 8) {
        for(y = 0; y < height; y++) {
          ...
          for(x = 0; x < 8; x++) {
            ...
            value >>= 2;
            sprites[i][y][width * 2 - 1 - x - offset]
                = sprites[i][y][x + offset]
                    = (color == 0) ? -1
                    : (color == 1) ? 0
                    : (color == 2) ? PALETTE[palette0]
                    : PALETTE[palette1];
          }
        }
      }
    ...
    for(y = 0; y < 8; y++) {
      for(x = 0; x < 8; x++) {
        if (sprites[SPRITE_FIRE][y << 1][x << 1] >= 0) {
          sprites[SPRITE_MAN_SHOOTING][y + 5][x + 4]
              = sprites[SPRITE_FIRE][y << 1][x << 1];
        }
      }
    }
    ...
    for(y = 0; y < 14; y++) {
      for(x = 0; x < 30; x++) {
        for(i = 0; i < 4; i++) {
          for(j = 0; j < 3; j++) {
            textures[TEXTURE_BRICK_WALL][(y + i * 16) & 63]
                [16 * (i&1) + (x + j * 32) & 63]
                    = (int)((1f - random.nextFloat() * 0.2f)
                        * (227 - 148 * y / 12f));
          }
        }
      }
    ...
    for(y = 0; y < 64; y++) {
      for(x = 0; x < 64; x++) {
        ...
        textures[TEXTURE_GRAY_SWASTIKA_WALL][y][x]
            = textures[TEXTURE_GRAY_BRICK_WALL][y][x]
                = (color << 16) | (color << 8) | color;
        textures[TEXTURE_SWASTIKA_WALL][y][x]
            = textures[TEXTURE_BRICK_WALL][y][x];
        color = (((x == 8 || x == 55) && y >= 8 && y <= 55)
            || ((y == 8 || y == 55) && x >= 8 && x <= 55)
                ? 0x59 : 0xBB - y - x) + random.nextInt(4);
        sprites[SPRITE_YELLOW_DOOR][y][x] = (color << 16) | (color << 8);
        sprites[SPRITE_BLUE_DOOR][y][x] = (color << 8) | color;

        sprites[SPRITE_BOSS][y][x] = sprites[SPRITE_MAN][y][x] == 0xD08050
            ? 0xFF0000 : sprites[SPRITE_MAN][y][x];
        sprites[SPRITE_BOSS_SHOOTING][y][x]
            = sprites[SPRITE_MAN_SHOOTING][y][x] == 0xD08050
                ? 0xFF0000 : sprites[SPRITE_MAN_SHOOTING][y][x];
        sprites[SPRITE_DEAD_BOSS][y][x]
            = sprites[SPRITE_DEAD_MAN][y][x] == 0xD08050
                ? 0xFF0000 : sprites[SPRITE_DEAD_MAN][y][x];
      }
    }
    ...
    for(y = 0; y < 32; y++) {
      for(x = 0; x < 32; x++) {
        if (sprites[SPRITE_SWASTIKA][y >> 2][x >> 2] >= 0) {
          textures[TEXTURE_GRAY_SWASTIKA_WALL][16 + y][16 + x]
              = textures[TEXTURE_SWASTIKA_WALL][16 + y][16 + x]
                  = sprites[SPRITE_SWASTIKA][y >> 2][x >> 2];
        }
      }
    }
    ...
    while(true) {
      ...
      do {
        ...
        if (playerFiring > 0) {
          ...
          if (playerTriggerReleased) {
            ...
            outter: while(map[(int)by >> 6][(int)bx >> 6] == 0) {
              bx += vx;
              by += vy;
              for(i = 0; i < objectMap[(int)by >> 6][(int)bx >> 6].size();
                  i++) {
                ...
                if (object[OBJ_TYPE] == TYPE_MAN) {
                  if (--object[OBJ_HITS] == 0) {
                    ...
                    object[OBJ_SPRITE] = object[OBJ_BOSS] == 1
                        ? SPRITE_DEAD_BOSS : SPRITE_DEAD_MAN;
        ...
        if (map[((int)(nextY + testY) >> 6) & 15]
                [((int)(nextX + testX) >> 6) & 15] == 0
            && map[((int)nextY >> 6) & 15]
                [((int)(nextX + testX) >> 6) & 15] == 0
            && map[((int)(nextY + testY) >> 6) & 15]
                [((int)nextX >> 6) & 15] == 0) {
          playerX = nextX;
          playerY = nextY;
        }
        ...
        for(i = queue.size() - 1; i >= 0; i--) {
          ...
          if (object[OBJ_REMOVE] == 1) {
            queue.remove(i);
            objectMap[(int)object[OBJ_Y] >> 6][(int)object[OBJ_X] >> 6]
                .remove(object);
            continue;
          }
          ...
          if (object[OBJ_TYPE] == TYPE_MAN) {
            ...
            if (object[OBJ_STATE] == MAN_STATE_PAUSED) {
              if (--object[OBJ_TIMER2] < 0) {
                object[OBJ_SPRITE] = object[OBJ_BOSS] == 1 
                    ? SPRITE_BOSS : SPRITE_MAN;
              }
              if (--object[OBJ_TIMER] < 0) {
                object[OBJ_TIMER] = 60 + object[OBJ_BOSS] == 1 
                    ? random.nextInt(60) : random.nextInt(180);
              ...
              if (map[((int)(object[OBJ_Y] + testY) >> 6) & 15]
                      [((int)(object[OBJ_X] + testX) >> 6) & 15] != 0
                  || map[((int)(object[OBJ_Y]) >> 6) & 15]
                      [((int)(object[OBJ_X] + testX) >> 6) & 15] != 0
                  || map[((int)(object[OBJ_Y] + testY) >> 6) & 15]
                      [((int)(object[OBJ_X]) >> 6) & 15] != 0
                  || --object[OBJ_TIMER] < 0) {
                object[OBJ_STATE] = MAN_STATE_PAUSED;
                object[OBJ_TIMER] = 30 + random.nextInt(30);
                object[OBJ_TIMER2] = 10;
                object[OBJ_SPRITE] = object[OBJ_BOSS] == 1 
                    ? SPRITE_BOSS_SHOOTING : SPRITE_MAN_SHOOTING;
                object[OBJ_VX] = 0;
                object[OBJ_VY] = 0;
          ...
          } else if (object[OBJ_TYPE] == TYPE_HORIZONTAL_DOOR
              || object[OBJ_TYPE] == TYPE_VERTICAL_DOOR) {
            ...
            if (object[OBJ_STATE] == DOOR_STATE_CLOSED) {
              ...
              if (dx * dx + dy * dy < 9216 && a[VK_SPACE] 
                  && (object[OBJ_HITS] == 0 || playerHasKey)) {
              ...
              if (--object[OBJ_TIMER] <= 0
                  && !(((int)playerX >> 6) == object[OBJ_X2]
                      && ((int)playerY >> 6) == object[OBJ_Y2])
                  && objectMap[(int)object[OBJ_Y2]][(int)object[OBJ_X2]].size()
                      == 1) {
                object[OBJ_TIMER] = 16;
          ...
          } else if (object[OBJ_TYPE] == TYPE_MED_PACK
              || object[OBJ_TYPE] == TYPE_KEY) {
            float dx = object[OBJ_X] - playerX;
            float dy = object[OBJ_Y] - playerY;
            if (dx * dx + dy * dy < 1024) {
          ...
          if (object[OBJ_VX] != 0 && object[OBJ_VY] != 0) {
            for(y = -1; y <= 1; y++) {
              for(x = -1; x <= 1; x++) {
                objectMap[((int)(object[OBJ_Y] + 16 * y) >> 6) & 15]
                    [((int)(object[OBJ_X] + 16 * x) >> 6) & 15]
                        .remove(object);
              }
            }
            ...
            for(y = -1; y <= 1; y++) {
              for(x = -1; x <= 1; x++) {
                if (!objectMap[((int)(object[OBJ_Y] + 16 * y) >> 6) & 15]
                    [((int)(object[OBJ_X] + 16 * x) >> 6) & 15]
                        .contains(object)) {
                  objectMap[((int)(object[OBJ_Y] + 16 * y) >> 6) & 15]
                      [((int)(object[OBJ_X] + 16 * x) >> 6) & 15]
          ...
          if (objects.size() > 0) {
            for(k = 0; k < objects.size(); k++) {
              ...
              if (S >= 0 && S < 1 && T > 0) { // verify hit
                int spriteX = (int)(object[OBJ_FLIPPED] == 1
                    ? object[OBJ_WIDTH] - object[OBJ_WIDTH] * S
                    : object[OBJ_WIDTH] * S);
                ...
                for(j = jMin; j <= jMax; j++) {
                  if (j >= 0 && j < 256 && zbuffer[i][j] > T) {
                    ...
                    if (spriteX >= 0 && spriteY >= 0 
                        && spriteX < object[OBJ_WIDTH]
                        && spriteY < object[OBJ_HEIGHT]
                        && sprites[(int)object[OBJ_SPRITE]][spriteY][spriteX]
                            >= 0) {
                      pixels[(j << 8) + i] 
                          = sprites[(int)object[OBJ_SPRITE]][spriteY][spriteX];
                      zbuffer[i][j] = T;
          ...
          if (j >= 0 && j < 256) {
            if (t < zbuffer[i][j]) {
              pixels[(j << 8) + i] 
                  = textures[lx > 8
                          ? ly < 8
                              ? TEXTURE_GRAY_BRICK_WALL
                              : TEXTURE_GRAY_SWASTIKA_WALL
                          : ly < 8
                              ? TEXTURE_BRICK_WALL
                              : TEXTURE_SWASTIKA_WALL]
                        [(int)textureY & 63][textureX];
            }
          }
        }
      }
      ...
      if (playerWinning > 0) {
        g.setColor(blacks[playerWinning >= 256
            ? 511 - playerWinning : playerWinning]);
        g.fillRect(0, 0, 256, 256);
      }
  ...
  public void processKeyEvent(KeyEvent keyEvent) {
    ...
    if (k > 0) {
      ...
      a[(k >= VK_LEFT && k <= VK_DOWN) || k == VK_SPACE ? k : VK_SHOOT]
          = keyEvent.getID() != 402;
    }
  }
