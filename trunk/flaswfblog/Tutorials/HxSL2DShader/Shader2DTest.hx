...
class Shader extends format.hxsl.Shader {
  ...
  static var SRC = {
    ...
    function vertex() {
      out = pos.xyzw;/*set vertex position*/ 
	  tuv = uv;
    }
    function fragment(tex : Texture, tex1 : Texture) {
       ...
       var c:Float4 = tex.get(tuv);//similar to Pixel Bender's sampleNearest(src,outCoord());
       ...
       out = tex1.get(tuv)*0.1 + c*0.9;/*set output color*/ //similar to Pixel Bender's dst =;
    }
  };
